...
export async function renderExcludeFolders(settingsTab: SettingsTab): Promise<void> {
  ...
  // eslint-disable-next-line max-len
  manageExcluded.infoEl.style.color =
  ...
  new Setting(containerEl).setName('Exclude folder default settings').addButton((cb) => {
    cb.setButtonText('Manage');
    cb.setCta();
    cb.onClick(async () => {
  ...
  new Setting(containerEl).setName('Exclude pattern default settings').addButton((cb) => {
    cb.setButtonText('Manage');
    cb.setCta();
    cb.onClick(async () => {
      new PatternSettings(
